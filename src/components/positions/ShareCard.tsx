  ...
  ({ ticker, variant, dark, lang, period }, ref) => {
    ...
    return (
      ...
      >
        ...
        {/* Ticker name + label */}
        <div>
          <div style={{ fontSize: 28, fontWeight: 800, letterSpacing: -1, color: text }}>
            {ticker.symbol}
          </div>
          ...
          return (
            ...
            >
              ...
              <span style={{ fontSize: 36, flexShrink: 0 }}>
                {variant === 'rate' ? (positive ? '🚀' : '📉') : positive ? '📈' : '📉'}
              </span>
            </div>
          )
        })()}
        ...
        >
          {variant === 'rate' ? (
            ...
            <>
              ...
              <CardRow
                label={t('mktValue')}
                value={ticker.marketValue > 0 ? `$${ticker.marketValue.toLocaleString()}` : '—'}
                muted={muted}
                text={text}
              />
            </>
