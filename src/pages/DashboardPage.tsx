...
function TopBar({ accountName, period, lang, darkMode, masked, isDemo, onUploadMore, onToggleLang, onToggleDark, onToggleMask }: TopBarProps) {
    ...
    <header className={`sticky top-0 z-10 ${bg} border-b ${borderColor} px-4 py-3`}>
      <div className="max-w-7xl mx-auto flex items-center gap-4">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={onToggleLang} className={`px-3 py-1.5 text-xs font-medium rounded-md transition-colors cursor-pointer ${btnSecondary}`}>
            {lang === 'en' ? '中文' : 'EN'}
          </button>
...
export default function DashboardPage() {
  ...
  const tabActive = darkMode ? 'text-white border-b-2 border-green-400' : 'text-gray-900 border-b-2 border-green-600'
